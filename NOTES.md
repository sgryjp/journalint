...
2. サーバーは、[`textDocument/didOpen` 通知][textDocument/didOpen]および
   ...
   チェックを行って [`Diagnostic`][lsp_types::Diagnostic] を作成
   し、[`textDocument/publishDiagnostics` 通
   知][textDocument/publishDiagnostics]でクライアントにそれらを報告する
3. クライアントは、報告された Diagnostic を UI で提示する。
   - VSCode の場合、エラーであれば赤い下波線が、警告であればオレンジ色の下波線
     が、そのエラーまたは警告の対象となる部分に対して引かれる。
4. クライアントは、ユーザーが Diagnostic のいずれかにマウスカーソルやテキスト
   カーソルを合わせると、その位置で使用可能な Code Action の一覧を
   [`textDocument/codeAction` 要求][textDocument/codeAction] メッセージでサー
   バーから取得する。このメッセージには、ファイルの URL、カーソル位置、その位置
   に該当する Diagnostic 一覧などが添えられる。
   - VSCode の場合、「豆電球 (Code Action lightbulb)」を表示して「そこで何らかの
     Code Action を実行できますよ」と教えてくれる。この UI を実現するために、
     カーソル位置が変更されるたびにサーバーへ一覧を要求するようだ。なお豆電球は
     設定で無効化できる。
...
- `workspace/executeCommand` で取り扱う「コマンド」は、一意な名前の付いたクライ
  アントが呼び出せる処理を指す。 多くの VSCode ユーザーはキーボードショートカッ
  トを設定するときに見かけていると思う（例: インデントを増やすコマンドの名前は
...
ばならないというのは不自然な話。調べると、CLI での report 表示用にファイル名と
ファイル内容を保持していただけだった。そして CLI でのレポート表示を行う文脈では
当該データが普通にアクセス可能な状態になっている。したがって、これらのデータは
Journalint インスタンスから削除するのが良いと思われる。

# 2024-02-21

これよりも前から気づいていた点だけれど、思い立ったので記録しておく。journalintの
場合はlsp_typesの型と同じ役割である専用の型を用意した方が良い。これは根本的には
journalintが使っているパーサーchumskyとの相性(?)に起因する話。というの
は、chumskyはパース対象データを1次元ベクトルとして扱うのでテキストの位置や範囲が
ファイル先頭からのオフセットで表す。これに対してlsp_typesもといLSPの仕様では、エ
ディタに寄せた仕様なのだと思うが、テキストの位置を行番号と行頭からのオフセットの
組で表している。これは非常に基礎的な部分での仕様差なので、もしlsp_typesの型を
使っていると多数の場面で行・列ペアとオフセットとの変換が必要になり、非常に煩雑に
なる。なので、基本戦略としてLSPの通信処理レイヤーで相互変換するよう徹底し、
journalint固有の処理は原則オフセット方式の処理で組んでおくと良い。なぜ良いとまで
言い切れるかと言うと、journalintを独立したコマンドラインプログラムとして構成する
ユースケースが普通に考えられ、その場合にlsp_typesに依存せずに済むから。

なお全面的にlsp_typesの型を使っておき、CLIでは擬似的にサーバーのイベントハンドラ
を呼ぶ (例えばtextDocument/codeActionのメッセージをプログラム的に作ってサーバー
のメッセージハンドラに渡ぶ) という方針も考えられる。こちらの方向でも、キチンと考
えれば綺麗に仕上がるかもしれない。ただ、何にせよ両タイプの型を無秩序に混ぜて使う
のは良くないということは確か。
