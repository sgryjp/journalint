...
これよりも前から気づいていた点だけれど、思い立ったので記録しておく。
journalintの場合はlsp_typesの型と同じ役割である専用の型を用意した方が良い。
これは根本的にはjournalintが使っているパーサーchumskyとの相性(?)に起因する話。
というのは、chumskyはパース対象データを1次元ベクトルとして扱うので
テキストの位置や範囲がファイル先頭からのオフセットで表す。
これに対してlsp_typesもといLSPの仕様では、
エディタに寄せた仕様なのだと思うが、
テキストの位置を行番号と行頭からのオフセットの組で表している。
これは非常に基礎的な部分での仕様差なので、
もしlsp_typesの型を使っていると多数の場面で行・列ペアと
オフセットとの変換が必要になり、非常に煩雑になる。
なので、基本戦略としてLSPの通信処理レイヤーで相互変換するよう徹底し、
journalint固有の処理は原則オフセット方式の処理で組んでおくと良い。
なぜ良いとまで言い切れるかと言うと、
journalintを独立したコマンドラインプログラムとして
構成するユースケースが普通に考えられ、
その場合にlsp_typesに依存せずに済むから。

なお全面的にlsp_typesの型を使っておき、
CLIでは擬似的にサーバーのイベントハンドラを呼ぶ
(例えばtextDocument/codeActionのメッセージを
プログラム的に作ってサーバーのメッセージハンドラに渡ぶ)
という方針も考えられる。こちらの方向でも、
キチンと考えれば綺麗に仕上がるかもしれない。
ただ、何にせよ両タイプの型を無秩序に混ぜて使うのは
良くないということは確か。
